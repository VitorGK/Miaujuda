    ...
    var body: some View {
        LazyVGrid(columns: columns, spacing: 10) {
            ForEach(0..<posts.count, id: \.self) { p in
                NavigationLink {
                    PostDetailsView(postItems: [ItemCard(itemName: posts[p].item.name, quantity: posts[p].item.quantity, category: posts[p].item.category, expDate: posts[p].item.expirationDate!)])
                } label: {
